...
3. **分派影子起草代理独立重写对比计划与差集对账（首波单审，拒绝顺向校对）**：
   ...
   - **黑盒先行派发构造（切断同源理解链）**：攻击性审查派发采用两段式——第一段只给 diff + "枚举什么输入能到达这段代码"的任务，不给动机、不给计划理由段，要求 Reviewer 先产出可证伪攻击清单（每项需求三段式：隐含假设 / 可证伪输入落到 `路径:行号` + 触发输入 / 爆炸半径）与不变量枚举（每个改动函数返回 / 状态域 × 消费点容忍性）；第二段才放理由供定性——攻击者必须在被说服之前完成攻击。派发 prompt 不得把"测试通过"框定为覆盖证据（只可作回归证据），并须声明千行级 diff 零攻击发现视为红旗、触发二轮换角度攻击而非直接 GO。

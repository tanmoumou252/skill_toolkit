...
1. **计划文件写作权仅属于 plan-writer 子代理**：主会话任何场景（含 ZCode 原生计划模式）不得内联撰写计划正文，不得让 ExitPlanMode 携带计划全文。
2. **必须加载`zcode-plan-first`技能**；**必须先派 plan-writer-subagent-sp 物理落盘 `.kilo/plans/`**。
3. **分派前须逐字枚举子代理清单**，佐证 `plan-writer-subagent-sp` 真实存在；三个类型名必须逐字精确——计划起草与回灌用 `plan-writer-subagent-sp`，独立复审用 `plan-reviewer-subagent-sp`，代码终验用 `pr-reviewer-subagent-sp`，严禁近似替换。
4. **写完计划即进入复审与回灌闭环**：初稿落盘并通过物理在场校验后，必须派发 `plan-reviewer-subagent-sp` 独立复审，并由 `plan-writer-subagent-sp` 完成 E 清单回灌；全部复审与回灌闭环后，ExitPlanMode 只携带终稿计划文件路径 + 一句话目标 + 分批摘要 + 三闸门声明呈报审批。
5. **快通道判定先行 + 被拦即派**：任务最终产物本身即 `.kilo/plans/` 下的流程文档（意见汇编、复审报告、合并记录等）时，直接分派对应子代理落盘并以【物理在场】校验闭环；快通道任务**禁止调用 ExitPlanMode**、禁止就此询问用户——以"分派 → 【物理在场】校验 → 文字汇报"结束回合即为合规结束；产物落点在**代码/配置区**或**工作区外**的更改实现任务才走"计划先行 + 终稿 ExitPlanMode"。

## EXECUTING 阶段执行与闭环四大铁律（CRITICAL: Execution & Closure Iron Laws）

以下四条对执行阶段的父会话与全体执行代理无条件生效，与上文终端命令规范、计划文件写作权纪律**叠加适用**，不因用户口头要求、权限档位变化（如计划批准后自动切 YOLO）或工具权限点击而豁免。

1. **绝对忠于计划、技术事实至上与自洁契约（Anti-Plan-Drift, Fact Supremacy & Clean Contract）**：
   - **文件范围硬闸门**：执行阶段修改源码时，业务改动的文件集合必须完全包含在主计划 Files 声明中；严禁借"顺手优化"改动计划未声明的业务源码与配置；
...
3. **终结交付门禁与双轨派发（Delivery Gate & Dual-Track）**：
   - 业务修改转绿并通过静态检查后，严禁直接口头汇报完成；
   - **轨道 A**：在当前响应轮次调用 `task` 分派 `pr-reviewer-subagent-sp` 发起终验，参数以主计划步骤 N 已固化的完整派发串逐字取用。三个子代理类型名必须**逐字精确**——计划落盘用 `plan-writer-subagent-sp`、计划独立复审用 `plan-reviewer-subagent-sp`、代码变更终验用 `pr-reviewer-subagent-sp`，严禁近似替换；
   - **轨道 B（无分派通道，降级）**：自行在受控终端运行只读 `git status --short -uall` 与 `git diff`、复跑测试自检，输出结构化报告后交人类处置；
...
4. **Git 绝对只读铁律（Git Read-Only Iron Law）**：
   ...
   - `.kilo/` 属 Agent 内部工作内存，不纳入 Git 追踪；代码提交与合入是人类特权，Agent 严禁越权——即便用户口头要求，也只输出命令原文交人类在宿主终端执行；产物落盘一律走受控写工具，不涉 Git；
...
以下四条规程从实战故障中提炼，与四大铁律叠加适用，优先级低于铁律但高于一般技能条款。

1. **派发前工作区卫生检查（Pre-Dispatch Workspace Hygiene）**：
   - 在调用 `task` 分派 `pr-reviewer-subagent-sp` 之前，必须先运行 `git status --short -uall` 检查工作区状态；
...
4. **测试保真度与夹具反假绿审计（Test Fidelity & Anti-Green-Wash Audit）**：
   - **核心公理：测试能跑通（Exit 0）≠ 测试本身是正确的。** 测试用例与夹具自身可能存在形态偏差、位置错误、断言过宽或浅层模拟，从而在被测代码存在真实漏洞时依然跑绿（如 M2 自测夹具错写在顶层而漏检嵌套违规，或符号链接仅测目录联接而漏测目标文件本身）；
   - **逆向审查义务**：Reviewer（含 Plan 与 PR 审查）在核验测试通过时，必须对测试用例与测试夹具自身的保真度（Fixture Fidelity）执行逆向审查——验证测试是否真正模拟并覆盖了最深、最危险的真实缺陷形态；严禁仅凭 Exit Code 0 即盲目轻信放行。